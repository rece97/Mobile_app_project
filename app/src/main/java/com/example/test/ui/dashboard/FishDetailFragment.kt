...
class FishDetailFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view =  inflater.inflate(R.layout.fragment_fish_detail, container, false)
        viewModel = ViewModelProvider(requireActivity()).get(FishViewModel::class.java)
        viewModel.selected.observe(viewLifecycleOwner, Observer<Fish> {
            ...
            acnhAPI.getFish(viewModel.selected.value!!.id).enqueue(object : Callback<Fish> {


                override fun onFailure(call: Call<Fish>, t: Throwable) {
                    Log.d(TAG, "onFailure: $t")
                }

                override fun onResponse(
                    call: Call<Fish>,
                    response: Response<Fish>
                ) {
